...
fn apply_unified_patch(content: String, patch_text: &str) -> Result<String> {
    ...
    for line in patch_text.lines() {
        if line.starts_with("---") || line.starts_with("+++") {
            ...
            if parts.len() < 3 {
                return Err(Error::InvalidParam(format!("Invalid hunk header: {}", line)));
            }
            ...
            hunk_old_start = old_parts[0].parse::<usize>()
                .map_err(|_| Error::InvalidParam(format!("Invalid line number in hunk header: {}", line)))?;
        } else if in_hunk {
            if line.starts_with(" ") {
                // Context line
                ...
                if line_num < patched_lines.len() && patched_lines[line_num] == text {
                    ...
                    return Err(Error::InvalidParam(format!(
                        ...
                        line_num + 1,
                        text,
                        if line_num < patched_lines.len() { &patched_lines[line_num] } else { "" }
                    )));
                }
            } else if line.starts_with("-") {
                // Remove line
...
fn apply_binary_patch(original: Vec<u8>, patch_text: &str) -> Result<Vec<u8>> {
    ...
    for line in patch_text.lines() {
        ...
        if parts.len() != 2 {
            return Err(Error::InvalidParam(format!(
                "Invalid binary patch line format: {}", line
            )));
        }
        
        // Parse the offset
        let offset = parts[0].parse::<usize>()
            .map_err(|e| Error::InvalidParam(format!(
                "Invalid offset in binary patch: {}", e
            )))?;
        
        // Decode the data
        let data = general_purpose::STANDARD.decode(parts[1])
            .map_err(|e| Error::InvalidParam(format!(
                "Invalid base64 data in binary patch: {}", e
            )))?;
