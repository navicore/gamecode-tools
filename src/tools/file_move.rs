    ...
    async fn execute(&self, params: Self::Params) -> Result<Self::Output> {
        ...
        if !source.exists() {
            return Err(Error::InvalidParam(format!(
                "Source not found: {}", params.source
            )));
        }
        ...
        if dest_exists && !params.overwrite {
            return Err(Error::InvalidParam(format!(
                "Destination already exists: {}", params.destination
            )));
        }
...
mod tests {
    ...
    fn get_test_dir() -> PathBuf {
        ...
        tmp_dir.join(format!("move_test_{}", chrono::Utc::now().timestamp_millis()))
    }
    ...
    async fn test_file_move_simple() -> Result<()> {
        ...
        // Verify setup
        assert!(source_file.exists(), "Source file doesn't exist before move");
    ...
    async fn test_file_move_overwrite() -> Result<()> {
        ...
        assert!(source_file.exists(), "Source file doesn't exist before move");
        assert!(dest_file.exists(), "Destination file doesn't exist before move");
        ...
        assert!(source_file.exists(), "Source file doesn't exist before second move attempt");
