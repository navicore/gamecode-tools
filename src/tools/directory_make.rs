...
impl Tool for DirectoryMake {
    ...
    async fn execute(&self, params: Self::Params) -> Result<Self::Output> {
        ...
        match result {
            ...
            Err(e) => match e.kind() {
                std::io::ErrorKind::NotFound => Err(Error::InvalidParam(format!(
                    "Parent directory does not exist: {}",
                    path.parent().unwrap_or(&path).display()
                ))),
                std::io::ErrorKind::PermissionDenied => Err(Error::PermissionDenied(format!(
                    "Permission denied: {}",
                    params.path
                ))),
                _ => Err(Error::Io(e)),
...
mod tests {
    ...
    async fn test_directory_make_existing() -> Result<()> {
        ...
        assert!(
            result.is_err(),
            "Creating an existing directory should fail with exist_ok=false"
        );
